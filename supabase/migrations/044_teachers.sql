...
-- Does this teacher hold this student? Used by every row-scoped policy below.
create or replace function public.teacher_holds_student(uid uuid, sid uuid)
returns boolean language sql stable security definer set search_path to 'public' as $$
  select exists(
    select 1 from public.teacher_students
    where teacher_id = uid and student_id = sid and is_active = true
  )
$$;
