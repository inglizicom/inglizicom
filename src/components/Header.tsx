        ...
        <nav className="hidden lg:flex items-center gap-1">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 hover:bg-brand-600/10 hover:text-brand-600 ${
        ...
        <div className="bg-white shadow-xl mx-4 mt-2 rounded-2xl p-4 border border-gray-100">
          {navLinks.map((link) => (
            <Link
              ...
              className="block px-4 py-3 rounded-xl text-gray-700 font-semibold hover:bg-brand-50 hover:text-brand-700 transition-colors"
            >
              {link.label}
            </Link>
          ))}
