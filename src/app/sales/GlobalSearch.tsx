...
/**
 * Global search bar used in the sales sidebar.
 * Searches leads + students; click result navigates to the right section.
 */
export default function GlobalSearch() {
  ...
  function pick(r: SearchResult) {
    if (r.type === 'lead')    router.push('/sales/leads')
    if (r.type === 'student') router.push('/sales/students')
  ...
  return (
    <div className="relative px-3 mb-1">
      ...
      {open && (
        ...
        >
          {loading && (
            <div className="py-5 flex justify-center text-gray-400"><Loader2 size={16} className="animate-spin" /></div>
          )}
            ...
            <ul className="divide-y divide-gray-100 max-h-72 overflow-y-auto">
              {results.map(r => (
                <li key={`${r.type}-${r.id}`}>
                  ...
                  >
                    <div className={`w-6 h-6 rounded flex items-center justify-center flex-shrink-0 ${r.type === 'student' ? 'bg-yellow-50 text-yellow-600' : 'bg-blue-50 text-blue-600'}`}>
                    ...
                    {r.status && (
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${LEAD_STATUS_META[normalizeStatus(r.status)]?.color ?? 'bg-gray-100 text-gray-500 border-gray-200'}`}>
                        {LEAD_STATUS_META[normalizeStatus(r.status)]?.short ?? r.status}
                      </span>
                    )}
                    {r.type === 'student' && (
                      <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-yellow-50 border border-yellow-200 text-yellow-800">Student</span>
                    )}
                  </button>
                </li>
              ))}
