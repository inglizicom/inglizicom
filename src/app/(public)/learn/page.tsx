...
const LESSONS: Lesson[] = [
  {
    ...
    exercises: [
      ...
      { type: 'mc',    tip_ar: 'كيف تحيي شخصاً في الصباح؟',          prompt: 'How do you greet in the morning?', options: ['Good evening','Goodbye','Good morning','Good night'],                    correct: 2 },
      ...
      { type: 'fill',  tip_ar: 'اختر الكلمة المناسبة',                prompt: 'Good ___, see you tomorrow.', options: ['morning','afternoon','evening','night'],                                          correct: 2 },
  ...
  },
  {
    ...
    exercises: [
      ...
      { type: 'fill',  tip_ar: 'اختر الكلمة المناسبة',                prompt: 'I want to ___ a coffee.',   options: ['drink','order','make','find'],                                                       correct: 1 },
...
function ProgressHeader({
  ...
  const phaseLabels: Record<Phase, string> = {
    vocab: 'المفردات', sentences: 'الجمل', dialogue: 'الحوار', exercises: 'التمارين', complete: 'منتهي'
  }
  ...
  return (
    <div className="shrink-0 px-4 pt-16 pb-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
      {/* Top row */}
      <div className="flex items-center justify-between mb-3">
        <button
          onClick={onBack}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-white/40 text-sm font-bold transition-all active:scale-95 hover:text-white/70"
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-white font-black text-sm" dir="rtl">{phaseLabels[phase]}</span>
        </div>

        <div className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-black text-white/50"
          style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}>
...
function VocabPhase({ lesson, onDone }: { lesson: Lesson; onDone: () => void }) {
  ...
  function prev() {
    if (index === 0) return
    setVisible(false)
    setTimeout(() => { setIndex(i => i - 1); setFlipped(false); setVisible(true) }, 180)
  }
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      <ProgressHeader
        phase="vocab"
        phaseStep={index + 1} phaseTotal={lesson.vocab.length}
        onBack={prev} color={lesson.color}
      />

      <div
        className="flex-1 flex flex-col items-center justify-between px-4 py-6 transition-opacity duration-180"
        style={{ opacity: visible ? 1 : 0 }}
      >
        {/* Card */}
        <button
          onClick={() => setFlipped(f => !f)}
          className="flex-1 w-full max-w-sm flex flex-col items-center justify-center gap-4 rounded-3xl border transition-all duration-300 active:scale-[0.97]"
          style={{
            background: flipped ? `${lesson.color}12` : 'rgba(255,255,255,0.03)',
            border: `2px solid ${flipped ? lesson.color + '50' : 'rgba(255,255,255,0.08)'}`,
          }}
        >
          ...
          >
            ...
            <p className="text-white/30 text-sm italic px-4 text-center">{word.example}</p>
          </div>

          {!flipped && (
            <p className="text-white/20 text-sm pb-4">اضغط لترى الترجمة</p>
          )}
        </button>
...
function SentencesPhase({ lesson, onDone }: { lesson: Lesson; onDone: () => void }) {
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      <ProgressHeader
        phase="sentences"
        phaseStep={index + 1} phaseTotal={lesson.sentences.length}
      ...
      >
        {/* Sentence card */}
        <div className="flex-1 w-full max-w-sm flex flex-col items-center justify-center gap-6">
          <div
            className="w-full rounded-3xl p-6 text-center border"
            style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}
          >
            <p className="text-white text-2xl font-bold leading-relaxed mb-4">{s.english}</p>
            ...
            {revealed ? (
              ...
              <button
                ...
                className="mt-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all active:scale-95"
              ...
              >
                <div className={`flex gap-3 ${isA ? '' : 'flex-row-reverse'}`}>
                  ...
                  <button
                    ...
                    className={`max-w-[75%] rounded-2xl p-3.5 text-left transition-all active:scale-[0.98] ${isA ? 'rounded-tl-sm' : 'rounded-tr-sm'}`}
                    style={{
                  ...
                  >
                    ...
                    {isRevealed && (
                      <p className="text-white/45 text-xs mt-1.5 leading-relaxed" style={{ color: isA ? lesson.color + 'aa' : '#818cf8aa' }} dir="rtl">
                        {line.arabic}
                      </p>
                    )}
                    {!isRevealed && isVisible && (
                      <p className="text-white/15 text-xs mt-1">اضغط للترجمة...</p>
                    )}
                  </button>
                </div>
                ...
                {/* Name label */}
                {isVisible && (
                  <p className={`text-white/20 text-xs mt-1 ${isA ? 'ml-13 pl-1' : 'mr-13 pr-1 text-right'}`}
                    style={{ marginLeft: isA ? 52 : 0, marginRight: isA ? 0 : 52 }}>
                    {line.name}
                  </p>
                )}
              </div>
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      ...
      <div
        ...
        style={{ background: 'rgba(255,255,255,0.03)', border: `1.5px dashed ${checked ? (correct ? 'rgba(16,185,129,0.5)' : 'rgba(239,68,68,0.5)') : 'rgba(255,255,255,0.12)'}` }}
      >
...
function ExercisesPhase({ lesson, onDone }: { lesson: Lesson; onDone: (score: number) => void }) {
  ...
  function handleNext() {
    ...
    setTimeout(() => {
      if (index < totalEx - 1) {
        ...
        onDone(score + (isCorrect ? 0 : 0))  // score already updated
      }
    }, 200)
  }
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      <ProgressHeader
        phase="exercises"
        phaseStep={index + 1} phaseTotal={totalEx}
        onBack={() => {}}
        color={lesson.color}
      />
        ...
        {ex.type !== 'order' && (
          ...
          >
            <p
              ...
              style={{ fontSize: ex.prompt.length > 30 ? 18 : 24, color: '#fff' }}
            >
              {ex.prompt}
            </p>
          </div>
...
function CompleteScreen({
  ...
  const msg   = pct === 100 ? 'نتيجة مثالية! أنت رائع!' : pct >= 80 ? 'ممتاز جداً! استمر هكذا!' : pct >= 60 ? 'جيد! التدريب يصنع الفارق' : 'لا بأس، كرر الدرس!'

  return (
    <div className="flex-1 flex flex-col items-center justify-center px-6 gap-6 text-center">
      <div
        className="w-28 h-28 rounded-3xl flex items-center justify-center text-6xl"
        style={{ background: `${lesson.color}18`, border: `2px solid ${lesson.color}40`, boxShadow: `0 0 40px ${lesson.color}30` }}
      >
        {medal}
      </div>
      ...
      <div dir="rtl">
        ...
        <p className="text-white/40 text-base">{msg}</p>
        <p className="text-yellow-400 font-bold text-sm mt-1">+{xpEarned} XP مكتسبة ⚡</p>
      </div>
      ...
      <div className="w-full max-w-xs" dir="rtl">
        ...
        <div className="flex flex-wrap gap-2 justify-center">
          {lesson.vocab.map((v, i) => (
            <span key={i} className="text-sm px-3 py-1.5 rounded-xl font-bold" style={{ background: `${lesson.color}12`, border: `1px solid ${lesson.color}30`, color: lesson.color }}>
              {v.emoji} {v.word}
            </span>
          ))}
        </div>
...
function LessonList({ onSelect }: { onSelect: (l: Lesson) => void }) {
  ...
  const byLevel = ['A1', 'A2', 'B1'].map(lvl => ({
    lvl,
  ...
  return (
    ...
    >
      ...
      <div className="max-w-sm mx-auto mb-8 text-center">
        ...
        <p className="text-white/30 text-sm" dir="rtl">٤ خطوات لكل درس — مفردات · جمل · حوار · تمارين</p>
        {totalXp > 0 && (
        ...
        {byLevel.map(({ lvl, lessons }) => (
          <div key={lvl}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-white/30 text-xs font-black px-2">مستوى {lvl}</span>
              <div className="h-px flex-1 bg-white/8" />
            </div>
                  ...
                  >
                    ...
                    <div className="flex-1" dir="rtl">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        {isDone && <span className="text-xs px-1.5 py-0.5 rounded-md bg-green-500/15 text-green-400 font-bold">✓</span>}
                      </div>
                      <p className="text-white/30 text-xs">{lesson.title} · {lesson.vocab.length} كلمات · {lesson.exercises.length} تمرين</p>
                      <div className="flex gap-1 mt-1.5 flex-wrap">
                        ...
                        {lesson.vocab.length > 3 && (
                          <span className="text-xs px-2 py-0.5 rounded-lg text-white/20">+{lesson.vocab.length - 3}</span>
                        )}
                      </div>
                    </div>
...
export default function LearnPage() {
  ...
  if (active) return <LessonPlayer lesson={active} onBack={() => setActive(null)} />
